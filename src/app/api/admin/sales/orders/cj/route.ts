...
function failure(error: unknown) {
  ...
  if (error instanceof CjRequestError) return { status: 502, message: "CJ no confirmó la creación. No reintentes de inmediato: busca primero la referencia Nexora en MyCJ para evitar duplicados." };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = buildCjCreateOrderV2Payload(order);
