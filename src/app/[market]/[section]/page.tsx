...
import { getOperationalCatalog, getStoreCatalog } from "@/lib/catalog-store";
...
export default async function LocalizedSectionPage({ params }: Props) {
  ...
  if (niche) {
    const copy = categoryCopy[market][niche];
    const products = (await getStoreCatalog(niche, market)).filter((product) => isStoreProductAvailable(product) && hasCompleteEditorial(product, market)).map((product) => toStorefrontProduct(product, market, exchangeRate));
