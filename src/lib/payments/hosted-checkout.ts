...
function getProvider(): Provider {
  ...
  // Mantiene compatibilidad con instalaciones previas: no hay SDK en cliente y no se exponen secretos.
...
export async function createHostedCheckout(product: Product, siteUrl: string): Promise<CheckoutSession> {
  ...
  return provider === "wompi"
    ? createWompiPaymentLink(product, siteUrl, externalReference)
...
async function createWompiPaymentLink(product: Product, siteUrl: string, externalReference: string): Promise<CheckoutSession> {
  ...
  const baseUrl = process.env.WOMPI_API_BASE_URL || (privateKey.includes("_test_") ? "https://sandbox.wompi.co" : "https://production.wompi.co");
  const response = await fetch(`${baseUrl}/v1/payment_links`, {
    ...
    body: JSON.stringify({
      ...
      redirect_url: `${siteUrl}/checkout/resultado?provider=wompi`,
    }),
    cache: "no-store",
  });
