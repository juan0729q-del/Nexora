...
export function IntelligenceProposalCard({ proposal, connected }: { proposal: IntelligenceProposal; connected: boolean }) {
  ...
  async function decide(decision: "authorized" | "rejected") {
    ...
    try {
      ...
      setMessage(decision === "authorized"
        ? proposal.execution === "catalog_workflow"
          ? "Autorización registrada. El cambio de catálogo queda en el flujo CJ versionado y no se considerará ejecutado hasta superar su validación."
          : "Autorización registrada y aplicada al flujo operativo controlado."
  ...
  return <article className="rounded-2xl border border-silver/15 bg-white/[.025] p-5">
    ...
    {!decided ? <div className="mt-5 space-y-3 border-t border-silver/10 pt-5"><label className="flex items-start gap-3 text-xs leading-5 text-silver/70"><input type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} className="mt-1 accent-[#009473]" />He leído beneficios, riesgos, implicaciones y mecanismo de reversión.</label><label className="block text-xs text-silver/60">Nota de decisión (opcional)<textarea value={note} onChange={(event) => setNote(event.target.value.slice(0, 800))} className="mt-2 min-h-20 w-full rounded-xl border border-silver/15 bg-onyx px-3 py-2 text-sm text-white outline-none focus:border-emerald" /></label><div className="flex flex-wrap gap-2"><button type="button" onClick={() => decide("authorized")} disabled={!connected || !acknowledged || Boolean(pending)} className="rounded-full bg-emerald px-4 py-2 text-sm font-bold text-onyx disabled:cursor-not-allowed disabled:opacity-40">{pending === "authorized" ? "Registrando…" : "Autorizar"}</button><button type="button" onClick={() => decide("rejected")} disabled={!connected || !acknowledged || Boolean(pending)} className="rounded-full border border-red-300/30 px-4 py-2 text-sm font-semibold text-red-100 disabled:cursor-not-allowed disabled:opacity-40">{pending === "rejected" ? "Registrando…" : "Rechazar"}</button></div></div> : <p className="mt-5 border-t border-silver/10 pt-4 text-xs text-silver/60">Decisión: <strong className="text-white">{proposal.status}</strong>{proposal.decisionNote ? ` · ${proposal.decisionNote}` : ""}</p>}
