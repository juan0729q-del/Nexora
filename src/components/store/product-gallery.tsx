 ...
 */
export function ProductGallery({ product, market = "co" }: { product: GalleryProduct; market?: Market }) {
  ...
  return (
    <section aria-label={market === "co" ? `Galería de ${product.name}` : `${product.name} gallery`} className="space-y-3">
      <div className="relative aspect-square overflow-hidden rounded-3xl border border-silver/20 bg-[#151515] shadow-2xl shadow-black/25">
        ...
        <span className="absolute right-4 bottom-4 rounded-full border border-white/15 bg-black/55 px-3 py-1.5 text-[11px] font-semibold tracking-[0.12em] text-white uppercase backdrop-blur-sm">
          {market === "co" ? "Imagen oficial CJ" : "Official CJ image"} {activeIndex + 1}/{images.length}
        </span>
      </div>

      {images.length > 1 && (
        <div className="grid grid-cols-5 gap-2 sm:grid-cols-6" aria-label={market === "co" ? "Seleccionar imagen del producto" : "Select product image"}>
          {images.map((image, index) => (
            <button
              key={image.src}
              type="button"
              aria-label={market === "co" ? `Ver imagen ${index + 1} de ${product.name}` : `View image ${index + 1} of ${product.name}`}
