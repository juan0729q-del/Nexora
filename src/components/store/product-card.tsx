    ...
    <div className="px-1 pb-2 pt-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-lg font-semibold text-white"><Link href={`/productos/${product.slug}`} className="hover:text-emerald">{product.name}</Link></h3>
        </div>
      ...
      <p className="mt-2 min-h-10 text-sm leading-5 text-silver/70">{product.description}</p>
      <div className="mt-5 flex items-end justify-between gap-3">
        <div><p className="text-lg font-semibold text-white">{formatCOP(product.price)}</p>{product.compareAtPrice && <p className="text-xs text-silver/45 line-through">{formatCOP(product.compareAtPrice)}</p>}</div>
        <button onClick={buy} disabled={!available || isPreparing} className="rounded-full bg-emerald px-4 py-2.5 text-sm font-bold text-onyx transition hover:bg-emerald/85 disabled:cursor-not-allowed disabled:bg-silver/30 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald">{purchaseLabel}</button>
      </div>
