...
export function StoreHeader({ market = "co" }: { market?: Market }) {
  ...
  return <header className="sticky top-0 z-30 border-b border-silver/15 bg-onyx/90 px-3 backdrop-blur-xl sm:px-8 lg:px-12">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between">
      <Link prefetch={false} href={markets[market].homePath} className="flex items-center gap-2 font-semibold tracking-[0.18em] text-white">
      ...
      <div className="flex min-w-0 items-center gap-1.5 sm:gap-3">
        ...
        <Link prefetch={false} href={cartPath(market)} aria-label={hydrated ? `${dictionary.cart}: ${itemCount} ${unitLabel}` : dictionary.cart} className="rounded-full border border-emerald/40 px-2.5 py-1.5 text-sm font-semibold text-white transition hover:bg-emerald hover:text-onyx sm:px-3">
